// namespace chess {
    ...
    * */
    ...
    export class MoveMaster {
        ...
        * */
        binMove(point: Point, side: SIDE): Point[] {
            // console.log(test);
            let back = [];
            let pass, //标志是否过河
                direction = 0, //方向
            ...
            if (point.y + direction >= 0 && point.y + direction <= this.map.length) {
                if (this.map[point.x][point.y + direction] * sign <= 0) {//如果没有棋子或者是敌方棋子的情况下(0),相乘为负数
                    let target = new Point(point.x, point.y + direction);
                    back.push(target);
                }
            }
            if (pass) {
        ...
        * 炮
        * */
        paoMove(point: Point, side: SIDE): Point[] {
            let back = [];
        ...
        private paoMove_(point: Point, direction: number, side: SIDE): Point[] {
            let back = [];
            ...
            while (true) {
                ...
                if (this.map[col][row] == 0) { //该位置没有棋子
                    if (!eat) back.push(new Point(col, row));
        ...
        juMove(point: Point, side: SIDE): Point[] {
            let back = [];

            for (let row = point.y; row >= 0; row--) { //行减遍历
                if (row == point.y) continue;
                if (!this.fastMove(new Point(point.x, row), side, back)) break;
            }
            for (let col = point.x; col >= 0; col--) {
                if (col == point.x) continue;
                if (!this.fastMove(new Point(col, point.y), side, back)) break;
            }
        ...
        private fastMove(point: Point, side: SIDE, back: Point[]): boolean {
            let sign = side ? 1 : -1;
        ...
        private maMove_(point: Point, side: SIDE, x, y): Point[] {
            let back = [],
                sign = side ? 1 : -1;
        ...
        xiangMove(point: Point, side: SIDE): Point[] {
            let back = [];
            //原理同马，四个撇脚位置
        ...
        private xiangMove_(point: Point, side, x, y): Point[] {
            let back = [];
        ...
        shiMove(point: Point, side: SIDE): Point[] {
            let back = [];
            //四个方向，方向规则不超过
        ...
        private shiMove_(point: Point, side: SIDE, x: number, y: number): Point[] {
            let back = [];
        ...
        bossMove(point: Point, side): Point[] {
            let back = [];
